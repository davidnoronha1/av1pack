...
av1pack runs completely client-side in the browser powered by **Preact**, the **WebCodecs VideoEncoder API**, and the **File System Access API**. Everything executes locally on your GPU/CPU with 0 bytes uploaded to any server.
...
- **Browser-Native ZIP Packing**: Standard PKZIP archive generation via `CompressionStream("deflate-raw")` and a small JS CRC-32 — no WASM required.
