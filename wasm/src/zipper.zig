...
pub fn Zipper(comptime Writer: type) type {
    return struct {
        ...
        pub fn writeFile(
            ...
            {
                var compressed_size: u32 = @intCast(opt.content.len);
                var uncompressed_size: u32 = @intCast(opt.content.len);
                var extra_len: u16 = 0;
                if (header_options) |hdr_options| {
                    if (hdr_options.compressed_size) |size| compressed_size = size;
                    if (hdr_options.uncompressed_size) |size| uncompressed_size = size;
                    if (hdr_options.extra_len) |len| extra_len = len;
                }
